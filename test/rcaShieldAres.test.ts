...
import { ISingleSidedInsurancePool } from "../src/types/ISingleSidedInsurancePool";
...
import { RcaShieldAres } from "../src/types/RcaShieldAres";
...
import { RcaShieldAres__factory } from "../src/types/factories/RcaShieldAres__factory";
...
describe("RcaShieldAres", function () {
  ...
  let unoToken: MockERC20;
  let aresPool: ISingleSidedInsurancePool;
  let unoWhale: SignerWithAddress;
  ...
  beforeEach(async function () {
    ...
    //   console.log('[signers]', _signers[i].address);
    // }
    // console.log('[other accounts]', signers, _signers.length)
    ...
    // await hre.network.provider.send("hardhat_impersonateAccount", [MAINNET_ADDRESSES.accounts.unoWhale]);
    signers.user = await ethers.getSigner(MAINNET_ADDRESSES.accounts.bitWethWhale);
    unoWhale = await ethers.getSigner(MAINNET_ADDRESSES.accounts.unoWhale);
    ...
    await signers.otherAccounts[0].sendTransaction({ to: unoWhale.address, value: ether("1000") });

    // btc weth pair
    contracts.uToken = <MockERC20>await ethers.getContractAt("MockERC20", MAINNET_ADDRESSES.contracts.ssip.unoToken);
    // uno token
    unoToken = <MockERC20>await ethers.getContractAt("MockERC20", MAINNET_ADDRESSES.contracts.ssip.unoToken);

    // ares pool
    aresPool = <ISingleSidedInsurancePool>(
      await ethers.getContractAt("ISingleSidedInsurancePool", MAINNET_ADDRESSES.contracts.ssip.ares)
    );
    ...
    const rcaShieldAresFactory = <RcaShieldAres__factory>await ethers.getContractFactory("RcaShieldAres");
    ...
    contracts.rcaShieldAres = <RcaShieldAres>(
      await rcaShieldAresFactory.deploy(
        "RcaShield Ares",
        "RcaAres",
    ...
    await contracts.rcaShieldAres.deployed();

    await contracts.rcaController.connect(signers.gov).initializeShield(contracts.rcaShieldAres.address);
    // Set liquidation tree.
    merkleTrees.liqTree1 = new BalanceTree([
      { account: contracts.rcaShieldAres.address, amount: ether("100") },
      { account: contracts.rcaController.address, amount: ether("100") },
    ]);
    // Set price tree.
    merkleTrees.priceTree1 = new BalanceTree([
      { account: contracts.rcaShieldAres.address, amount: ether("0.001") },
      { account: contracts.rcaController.address, amount: ether("0.001") },
      { account: contracts.uToken.address, amount: ether("0.001") },
      { account: unoToken.address, amount: ether("0.001") },
    ]);
    // Set reserved tree with 0 reserved.
    merkleTrees.resTree1 = new BalanceTree([
      { account: contracts.rcaShieldAres.address, amount: ether("0") },
      { account: contracts.rcaController.address, amount: ether("0") },
    ]);

    merkleProofs.liqProof1 = merkleTrees.liqTree1.getProof(contracts.rcaShieldAres.address, ether("100"));
    ...
    await contracts.uToken.connect(signers.user).approve(contracts.rcaShieldAres.address, ether("10000000"));
    await contracts.uToken.connect(signers.referrer).approve(contracts.rcaShieldAres.address, ether("10000000"));
  });
  async function mintTokenForUser() {
    const userAddress = signers.user.address;
    const uAmount = ether("1000");
    const shieldAddress = contracts.rcaShieldAres.address;
  ...
  //     expect(await contracts.rcaShieldAres.SSIPContract()).to.be.equal(aresPool.address);
  //   });
  // });
  ...
  //     //   userAddress,
  //     //   contracts.rcaShieldAres.address,
  //     // );
  ...
  //     let rcaBal = await contracts.rcaShieldAres.balanceOf(userAddress);
  ...
  //     let shieldDepositInfo = await aresPool.userInfo(shieldAddress);
  ...
  //     // Check for the RCA value recieved against uAmount
  //     rcaBal = await contracts.rcaShieldAres.balanceOf(userAddress);
  //     expect(rcaBal).to.be.equal(expectedRcaValue);

  //     // details of the deposited bitWeth lp and it's rewards
  //     // returns deposited amount and rewardDebt
  //     shieldDepositInfo = await aresPool.userInfo(shieldAddress);
  ...
  //     await mine();
  //     const shieldSushiBalanceBefore = await unoToken.balanceOf(shieldAddress);
  //     await contracts.rcaShieldAres.getReward();
  //     const shieldSushiBalanceAfter = await unoToken.balanceOf(shieldAddress);
  ...
  describe("purchase()", function () {
    it("should not allow user to buy underlying token", async function () {
      ...
      await unoToken.connect(unoWhale).transfer(shieldAddress, ether("1000"));
      ...
      */
      ...
      const sushiPriceProof = merkleTrees.priceTree1.getProof(unoToken.address, sushiPrice);
      // buy sushi reward token for referrer signer
      const sushiAmtToBuy = ether("100");
      await expect(
        contracts.rcaShieldAres
          .connect(signers.referrer)
          .purchase(
            contracts.uToken.address,
            sushiAmtToBuy,
    ...
    it("should allow user to buy harvested sushi and deposit uTokens to sushi masterchef", async function () {
      ...
      */
      ...
      const referrerSushiBalBefore = await unoToken.balanceOf(userAddress);
      const shieldDepositInfoBefore = await aresPool.userInfo(shieldAddress);
      console.log('[purchase]', unoToken.address, contracts.uToken.address);
      await contracts.rcaShieldAres
        .connect(signers.referrer)
        .purchase(unoToken.address, sushiAmtToBuy, sushiPrice, sushiPriceProof, underLyingPrice, underLyingPriceProof);

      const referrerSushiBalAfter = await unoToken.balanceOf(userAddress);
      const shieldDepositInfoAfter = await aresPool.userInfo(shieldAddress);
    ...
    it("should allow user to buy harvested sushi on discount", async function () {
      ...
      */
      ...
      // reward token balance of user purchasing it should increase
      const discount = await contracts.rcaShieldAres.discount();
  ...
  describe("redeemRequest()", function () {
    it("should withdraw lp tokens from sushi masterchef", async function () {
      ...
      // redeem request
      await contracts.rcaShieldAres.connect(signers.user).redeemRequest(rcaAmount, 0, [], 0, []);
